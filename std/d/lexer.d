...
//these should be in std.ascii, using tabulation there is so 90-ies

bool isAsciiWhite(uint x)
{
	x -= 0x09;
	enum high = 0x0D-0x09;
	enum  space = 0x20-0x09;
	return x  <= high || x == space;
}

bool isAsciiDigit(uint x)
{
	x -= '0';
	enum lastNum = '9' - '0';
	return x <= lastNum;
...
//        pragma(msg, generateCaseTrie(
    ...
    in
    {
        assert (isAsciiWhite(currentElement()));
    }
    body
    {
        ...
        {
            keepChar();
        }while (!isEoF() && isAsciiWhite(currentElement()));
        if (config.iterStyle & IterationStyle.includeWhitespace)
            setTokenValue();
    }
    ...
    body
    {
        ...
        while (true)
        {
            ...
            else if (isAsciiWhite(currentElement()) && (config.tokenStyle & TokenStyle.notEscaped))
            {
                keepChar();
            }
            else if (currentElement() == '"')
    ...
    in
    {
        assert(isAsciiDigit(cast(char) currentElement()) || currentElement() == '.');
    }
    body
    {
