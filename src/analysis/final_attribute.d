...
public:
	...
	{
		if (_parent == Parent.class_ && decl.attribute && decl.attribute.attribute == tok!"static")
			_alwaysStatic = true;
	}
	...
	override void visit(const(Declaration) d)
	{
		...
		if (!d.attributeDeclaration && !d.classDeclaration && !d.structDeclaration
				&& !d.unionDeclaration && !d.interfaceDeclaration && !d.functionDeclaration)
			return;
		...
		const bool isStaticOnce = d.attributes.canFind!(a => a.attribute.type == tok!"static");

		// determine if private
		const bool changeProtectionOnce = d.attributes.canFind!(a => a.attribute.type.isProtection);

		const bool isPrivateOnce = d.attributes.canFind!(a => a.attribute.type == tok!"private");
...
@system unittest
{
    ...
    }c.format(FinalAttributeChecker.MSGB.format(
			FinalAttributeChecker.MESSAGE.func_g)), sac);

	assertAnalyzerWarnings(q{
        void foo(){final void foo(){}} // [warn]: %s
    }c.format(FinalAttributeChecker.MSGB.format(
			FinalAttributeChecker.MESSAGE.func_n)), sac);
	...
	assertAnalyzerWarnings(q{
        void foo()
        {
			...
			FinalAttributeChecker.MESSAGE.class_f)), sac);

	assertAnalyzerWarnings(q{
        final struct Foo{} // [warn]: %s
    }c.format(FinalAttributeChecker.MSGB.format(
			FinalAttributeChecker.MESSAGE.struct_i)), sac);

	assertAnalyzerWarnings(q{
        final union Foo{} // [warn]: %s
    }c.format(FinalAttributeChecker.MSGB.format(
			FinalAttributeChecker.MESSAGE.union_i)), sac);

	assertAnalyzerWarnings(q{
        class Foo{private final void foo(){}} // [warn]: %s
    }c.format(FinalAttributeChecker.MSGB.format(
			FinalAttributeChecker.MESSAGE.class_p)), sac);
	...
	assertAnalyzerWarnings(q{
        interface Foo{final void foo(T)(){}} // [warn]: %s
			...
			FinalAttributeChecker.MESSAGE.interface_t)), sac);
	...
	assertAnalyzerWarnings(q{
        class Foo {final static void foo(){}} // [warn]: %s
			...
			FinalAttributeChecker.MESSAGE.class_s)), sac);
