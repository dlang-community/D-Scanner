...
public:
	...
	override void visit(const(FunctionDeclaration) decl)
	{
		...
		const bool autoFun = decl.storageClasses.any!(a => a.token.type == tok!"auto");
	...
	bool stringliteralHasReturn(const(NonVoidInitializer) nvi)
	{
		...
		if (u.primaryExpression && u.primaryExpression.primary.type.isStringLiteral
	...
	override void visit(const(AutoDeclaration) decl)
	{
		...
		foreach (const(AutoDeclarationPart) p; decl.parts)
			if (p.initializer && p.initializer.nonVoidInitializer
	...
	override void visit(const(VariableDeclaration) decl)
	{
		...
		foreach (const(Declarator) d; decl.declarators)
			if (d.initializer && d.initializer.nonVoidInitializer
...
unittest
{
	...
	}c.format(AutoFunctionChecker.MESSAGE,
			AutoFunctionChecker.MESSAGE, AutoFunctionChecker.MESSAGE,), sac);

	assertAnalyzerWarnings(q{
		auto doStuff(){assert(true);} // [warn]: %s
		auto doStuff(){assert(false);}
	}c.format(AutoFunctionChecker.MESSAGE,), sac);
